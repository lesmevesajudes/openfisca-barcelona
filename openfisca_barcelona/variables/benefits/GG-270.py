...
class solicitant_GG_270_valid(Variable):
    ...
    def formula(persona, period, parameters):
        ...
        es_empadronat_a_catalunya = (persona("municipi_empadronament", period) == b'barcelona') \
        + (persona("municipi_empadronament", period) == b'municipis_atm') \
        + (persona("municipi_empadronament", period) == b'municipis_altres')
        ...
        ingressos_mensuals_familia = persona.familia_fins_a_segon_grau.sum(
            persona.familia_fins_a_segon_grau.members("ingressos_bruts_ultims_sis_mesos", period)
        ) / 6
